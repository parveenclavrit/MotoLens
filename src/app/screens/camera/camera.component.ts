...
export class CameraComponent implements OnInit {
  ...
  takePicture = async () => {
    try {
      ...
      this.vehicleService.setmediaSource(imageUrl,"image","image.jpeg");
...
console.log(imageUrl);
  ...
  reader.onload = () => {
    // reader.result contains the Data URI
    const dataUri: string = reader.result as string;
    // Sanitize the data URI
    const safeDataUri: SafeUrl = this.sanitizer.bypassSecurityTrustUrl(dataUri);
    
    // Use the sanitized data URI
    console.log('Data URI:', safeDataUri);
  };
  ...
  async captureVideo() {
    ...
    // if (cameraPermission.hasPermission && audioPermission.hasPermission) {
      ...
      this.mediaCapture.captureVideo(options).then(async (res: any) => {
        console.log(res);
        if (res) {
          let win: any = window;
          var video = win.Ionic.WebView.convertFileSrc(res[0].base64);
          console.log("parveen" + video);
